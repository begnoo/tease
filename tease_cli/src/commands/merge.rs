use crate::utils::lines::{get_content_from_sha1, Line};
...
impl Display for Chunk {
    fn fmt(&self, f: &mut Formatter) -> Result {
        match self.resolve_type {
            ...
            ResolveType::Conflict => write!(f, ">>>>>>>>(incoming)>>>>>>>>\n{}\n================\n{}\n<<<<<<<<<<(current)<<<<<<<",
                                             self.a_lines.join("\n"), self.b_lines.join("\n"))
        }
    }
}
...
pub fn merge_file(a_sha1: String, b_sha1: String, o_sha1: String) {
    ...
    let chunks = generate_chunks(&a_matches, &b_matches, & mut match_index);
...
fn emit_final(match_index: & mut MatchIndex) -> Chunk {
    ...
    let o_chunk = map_chunk(&match_index.o_lines, match_index.o, match_index.o_len - match_index.o);
    let a_chunk = map_chunk(&match_index.a_lines, match_index.a, match_index.a_len - match_index.a);
    let b_chunk = map_chunk(&match_index.b_lines, match_index.b, match_index.b_len - match_index.b);
