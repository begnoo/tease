use crate::utils::blob_writer::{create_tease_file, tease_file_exists, update_current_branch, read_head};
use std::path::Path;
...
pub fn switch_to_branch(name: String) -> () {
    ...
    match update_current_branch(branch_head) {
        Ok(_) => println!("Switched to branch {:?}", name.to_string()),
