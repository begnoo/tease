...
use crate::remote_req::get_clone::get_clone;

pub fn clone(origin: String) {
    let repo_name = origin.split("/").last().unwrap();
    let res = get_clone(origin.to_string());
    if res.is_err() {
    ...
    execute_go_back_shell(repo_name.to_string(), head_commit.to_string());
}

fn execute_go_back_shell(repo_name: String, head_commit: String) {
